angular.module('plaza').service('entityService', function($http, $timeout, $rootScope, eventService) {
    ...
    function organize(entity, parent) {
        ...
        if(parent !== undefined) {
            ...
            entity.$path = (parent.$path !== undefined) ? (parent.$path + "/" + entity.Name) : "/" + entity.Name;    
        } else {
        ...
        for(var key in entity) {
            ...
            if(Array.isArray(value)) {
                ...
                children.$path = entity.$path + "/" + key;
                children.$parent = entity;
                children.$type = "folder";

                for(var i = 0; i < value.length; ++i) {
                    organize(value[i], children, key);
                }
            }
        }
    }
            ...
            return service.getEntity(scope, "/Modules/Debug/EntityTracker").then(function(tracker) {
                ...
                for(var i = 0; i < modifiedEntities.length; ++i) {
                    (function(i) {
                        ...
                        getJobs.push(service.getEntity(scope, path).then(function() {
                            return service.deleteEntity(scope, modificationPath);
                        }));
                    })(i);
                }

                if(scope.connection.selectedEntity && scope.connection.selectedEntity.$path) {
                    getJobs.push(service.getEntity(scope, scope.connection.selectedEntity.$path));
                }
