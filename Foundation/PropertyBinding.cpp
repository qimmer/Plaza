...
static void OnSourceEntityDataChanged(Entity entity, Type changedComponent) {
    ...
    for(auto& binding : bindings) {
        ...
        if(GetPropertyComponent(data->PropertyBindingSourceProperty) != changedComponent) continue;
...
static void OnDestinationEntityDataChanged(Entity entity, Type changedComponent) {
    ...
    for(auto& binding : bindings) {
        ...
        if(GetPropertyComponent(data->PropertyBindingDestinationProperty) != changedComponent) continue;
...
static void OnPropertyBindingSourceEntityChanged(Entity entity, Entity oldValue, Entity newValue) {
    ...
    if(IsEntityValid(oldValue)) {
        ...
        for (auto i = 0; i < EntitySourceBindings[oldEntityIndex].size(); ++i) {
            if(EntitySourceBindings[oldEntityIndex][i] == entity) {
                ...
                UnsubscribeEvent(EventOf_ComponentChanged(), oldValue, (void*)OnSourceEntityDataChanged);
                break;
            }
        }
    }
    ...
    if(IsEntityValid(newValue)) {
        ...
        SubscribeEvent(EventOf_ComponentChanged(), newValue, (void*)OnSourceEntityDataChanged);
    }
}
...
static void OnPropertyBindingDestinationEntityChanged(Entity entity, Entity oldValue, Entity newValue) {
    ...
    if(IsEntityValid(oldValue)) {
        ...
        for (auto i = 0; i < EntityDestinationBindings[oldEntityIndex].size(); ++i) {
            if(EntityDestinationBindings[oldEntityIndex][i] == entity) {
                ...
                UnsubscribeEvent(EventOf_ComponentChanged(), oldValue, (void*)OnDestinationEntityDataChanged);
                break;
            }
        }
    }
    ...
    if(IsEntityValid(newValue)) {
        ...
        SubscribeEvent(EventOf_ComponentChanged(), newValue, (void*)OnDestinationEntityDataChanged);
    }
}
...
static void OnEntityDestroyed(Entity entity) {
    ...
    if(EntityDestinationBindings.IsValid(entityIndex)) {
        for(auto& binding : EntityDestinationBindings[entityIndex]) {
            UnsubscribeEvent(EventOf_ComponentChanged(), entity, (void*)OnDestinationEntityDataChanged);
        }
...
void Bind(Entity destinationEntity, Property destinationProperty, Entity sourceEntity, Property sourceProperty) {
    auto bindings = CreateHierarchy(destinationEntity, "Bindings");

}

void Unbind(Entity destinationEntity, Property destinationProperty) {

}

Entity GetBinding(Entity destinationEntity, Property destinationProperty) {
    return 0;
}
